...
export interface DefiLpRecord { strategyId:string; accountId:string; componentInstrumentIds:[string,string]; investedAmount:string; currency:string; openedAt:string; sourceType:"MANUAL" }
export interface DefiLpSnapshotRecord { operationId:string; currentPositionValue:string; unclaimedFees:string|null; currency:string; observedAt:string; idempotencyKey:string }
...
export async function insertDefiLp(db:PoolClient,input:DefiLpRecord):Promise<string>{const id=randomUUID();await db.query(`INSERT INTO operations (id,strategy_id,account_id,template_type,template_version,operation_type,status,opened_at,source_type,invested_amount,metadata) VALUES ($1,$2,$3,'DEFI_LP',1,'LP_ENTRY','OPEN',$4,$5,$6::numeric,'{"pairIdentity":"WBTC/SOL","poolIdentityStatus":"INCOMPLETE"}'::jsonb)`,[id,input.strategyId,input.accountId,input.openedAt,input.sourceType,input.investedAmount]);for(const instrumentId of input.componentInstrumentIds)await db.query(`INSERT INTO operation_legs (id,operation_id,instrument_id,side,quantity,entry_price,currency,metadata) VALUES ($1,$2,$3,'BUY',NULL,NULL,$4,'{"role":"LP_COMPONENT","quantityStatus":"UNKNOWN"}'::jsonb)`,[randomUUID(),id,instrumentId,input.currency.toUpperCase()]);return id;}
export async function insertDefiLpSnapshot(db:PoolClient,input:DefiLpSnapshotRecord):Promise<string>{const id=randomUUID();await db.query(`INSERT INTO defi_lp_snapshots (id,operation_id,current_position_value,unclaimed_fees,currency,observed_at,idempotency_key) VALUES ($1,$2,$3::numeric,$4::numeric,$5,$6,$7)`,[id,input.operationId,input.currentPositionValue,input.unclaimedFees,input.currency.toUpperCase(),input.observedAt,input.idempotencyKey]);return id;}
export async function findLatestDefiLpSnapshot(db:Queryable,operationId:string){const result=await db.query<{id:string;operation_id:string;current_position_value:string;unclaimed_fees:string|null;currency:string;observed_at:Date;recorded_at:Date}>(`SELECT id,operation_id,current_position_value::text,unclaimed_fees::text,currency,observed_at,recorded_at FROM defi_lp_snapshots WHERE operation_id=$1 ORDER BY observed_at DESC, recorded_at DESC LIMIT 1`,[operationId]);return result.rows[0]??null;}
export async function listDefiLpSnapshots(db:Queryable,operationId:string){const result=await db.query<{id:string;operation_id:string;current_position_value:string;unclaimed_fees:string|null;currency:string;observed_at:Date;recorded_at:Date}>(`SELECT id,operation_id,current_position_value::text,unclaimed_fees::text,currency,observed_at,recorded_at FROM defi_lp_snapshots WHERE operation_id=$1 ORDER BY observed_at ASC, recorded_at ASC`,[operationId]);return result.rows;}
...
export async function findOperationById(
  ...
  const result = await db.query<{
    ...
    SELECT
      o.id, o.status, o.strategy_id, s.name AS strategy_name, o.template_type, o.template_version, o.closed_at, o.invested_amount::text, o.metadata,
