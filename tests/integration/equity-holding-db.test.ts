...
maybeTest("E2E Crypto Spot PostgreSQL precision and idempotency",async()=>{const{createApp}=await import("../../apps/api/src/app.js");const server=createApp().listen(0);const db=new pg.Client({connectionString:url});await db.connect();try{const address=server.address();assert.ok(address&&typeof address!=="string");const base=`http://127.0.0.1:${address.port}`;const[strategies,accounts,instruments]=await Promise.all(["/v1/strategies","/v1/strategies/accounts","/v1/strategies/instruments"].map(p=>fetch(base+p).then(r=>r.json())));const strategy=(strategies as {items:{id:string;code:string;templateType:string;templateVersion:number}[]}).items.find(x=>x.code==="STR-005");const wrongStrategy=(strategies as {items:{id:string;code:string}[]}).items.find(x=>x.code==="STR-001");const account=(accounts as {items:{id:string}[]}).items[0];const instrument=(instruments as {items:{id:string;symbol:string;currency:string}[]}).items.find(x=>x.symbol==="BTC");const equity=(instruments as {items:{id:string;symbol:string}[]}).items.find(x=>x.symbol==="EMBR3");assert.ok(strategy&&wrongStrategy&&account&&instrument&&equity);assert.equal(strategy.templateType,"CRYPTO_SPOT");assert.equal(strategy.templateVersion,1);const payload={strategyId:strategy.id,accountId:account.id,instrumentId:instrument.id,side:"BUY",quantity:"0.00031",unitPrice:"63000.38",currency:"USD",openedAt:"2026-02-01T10:00:00.000Z",template:{type:"CRYPTO_SPOT",version:1}};const previewResponse=await fetch(base+"/v1/operations/preview",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify(payload)});assert.equal(previewResponse.status,200);const preview=await previewResponse.json() as {metrics:{grossAmount:{value:string;currency:string};marketValue:{status:string;reason:string};unrealizedPnl:{status:string;reason:string}}};assert.deepEqual(preview.metrics.grossAmount,{status:"AVAILABLE",value:"19.5301178",currency:"USD"});assert.deepEqual(preview.metrics.marketValue,{status:"INCOMPLETE",value:null,reason:"MISSING_CURRENT_VALUATION"});assert.deepEqual(preview.metrics.unrealizedPnl,{status:"INCOMPLETE",value:null,reason:"MISSING_CURRENT_VALUATION"});const sellPreview=await fetch(base+"/v1/operations/preview",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({...payload,side:"SELL"})});assert.equal(sellPreview.status,422);const key=crypto.randomUUID(),headers={"content-type":"application/json","Idempotency-Key":key};const first=await fetch(base+"/v1/operations",{method:"POST",headers,body:JSON.stringify(payload)});assert.equal(first.status,201);const saved=await first.json() as {id:string};const retry=await fetch(base+"/v1/operations",{method:"POST",headers,body:JSON.stringify(payload)}).then(r=>r.json()) as {id:string};assert.equal(retry.id,saved.id);const conflict=await fetch(base+"/v1/operations",{method:"POST",headers,body:JSON.stringify({...payload,quantity:"0.00032"})});assert.equal(conflict.status,409);const invalidInstrument=await fetch(base+"/v1/operations",{method:"POST",headers:{...headers,"Idempotency-Key":crypto.randomUUID()},body:JSON.stringify({...payload,instrumentId:equity.id})});assert.equal(invalidInstrument.status,422);const mismatch=await fetch(base+"/v1/operations",{method:"POST",headers:{...headers,"Idempotency-Key":crypto.randomUUID()},body:JSON.stringify({...payload,strategyId:wrongStrategy.id})});assert.equal(mismatch.status,422);const reload=await fetch(base+"/v1/operations/"+saved.id).then(r=>r.json()) as {templateType:string;legs:{symbol:string;side:string;quantity:string;entryPrice:string;currency:string}[]};assert.equal(reload.templateType,"CRYPTO_SPOT");assert.equal(reload.legs[0]?.symbol,"BTC");assert.equal(reload.legs[0]?.side,"BUY");assert.ok(new Decimal(reload.legs[0]!.quantity).eq("0.00031"));assert.ok(new Decimal(reload.legs[0]!.entryPrice).eq("63000.38"));assert.equal(reload.legs[0]?.currency,"USD");assert.equal(new Decimal(reload.legs[0]!.quantity).mul(reload.legs[0]!.entryPrice).toString(),"19.5301178");const position=await fetch(`${base}/v1/operations/position/${account.id}/${instrument.id}`).then(r=>r.json()) as {quantity:string;costBasis:{value:string;currency:string};marketValue:{status:string;reason:string};unrealizedPnl:{status:string;reason:string}};assert.ok(new Decimal(position.quantity).eq("0.00031"));assert.ok(new Decimal(position.costBasis.value).eq("19.5301178"));assert.equal(position.costBasis.currency,"USD");assert.equal(position.marketValue.status,"INCOMPLETE");assert.equal(position.marketValue.reason,"MISSING_CURRENT_VALUATION");assert.equal(position.unrealizedPnl.status,"INCOMPLETE");const counts=await db.query<{operations:string;legs:string}>(`SELECT (SELECT count(*)::text FROM operations WHERE id=$1) operations,(SELECT count(*)::text FROM operation_legs WHERE operation_id=$1) legs`,[saved.id]);assert.equal(counts.rows[0]?.operations,"1");assert.equal(counts.rows[0]?.legs,"1");}finally{await new Promise<void>(resolve=>server.close(()=>resolve()));await db.end();}});
